...
def Compare_methods(X, y, X_test, X_sample_no, fn, feature_imp):
    ...
    y_tensor_normalized = (y_tensor - torch.mean(y_tensor)) / torch.std(y_tensor)

    featuregumbelsparsemax_model = HSICFeatureNetGumbelSparsemax(input_dim, feature_layers, act_fun_featlayer, layers, act_fun_layer, sigma_init_X, sigma_init_Y, num_samples*10)
    featuregumbelsparsemax_model.train_model(X_tensor, y_tensor, num_epochs=300, BATCH_SIZE = 1000)
    weights = featuregumbelsparsemax_model(X_tensor)[0]

    sigmas = featuregumbelsparsemax_model.sigmas
    sigma_y = featuregumbelsparsemax_model.sigma_y
    ...
    feature_importance_l2x = train_L2X(X_train, y_train, 10, 2, X, epochs=200)

    # Initialize the custom model
    model = CustomModel(fn)
    Invase_explainer = INVASE(model, X_df, y_series, n_epoch=1000, prefit=False)
    invase_scores =(Invase_explainer.explain(X_df)).to_numpy()                      

    
    featuregumbelsparsemax_model = HSICFeatureNetGumbelSparsemax(input_dim, feature_layers, layers, sigma_init_X, sigma_init_Y, num_samples)
    featuregumbelsparsemax_model.train_model(X_tensor, y_tensor)
    sigmas = featuregumbelsparsemax_model.sigmas
    sigma_y = featuregumbelsparsemax_model.sigma_y
    weights = featuregumbelsparsemax_model.importance_weights

    #HSICNetGumbelSparsemax
    gumbelsparsemax_model = HSICNetGumbelSparsemax(input_dim, layers, sigma_init_X, sigma_init_Y, num_samples)
    gumbelsparsemax_model.train_model(X_tensor, y_tensor)
    sigmas = gumbelsparsemax_model.sigmas
    sigma_y = gumbelsparsemax_model.sigma_y
    ...
    gumbelsparsemax_model2 = HSICNetGumbelSparsemax(input_dim, layers, sigma_init_X, sigma_init_Y, num_samples+3)
    ...
    gumbelsoftmax_model = HSICNetGumbelSoftmax(input_dim, layers, sigma_init_X, sigma_init_Y, num_samples)
    ...
    gumbelsoftmax_model2 = HSICNetGumbelSoftmax(input_dim, layers, sigma_init_X, sigma_init_Y, 2*num_samples)
    ...
    #HSICNetSparseMax
    sparsemax_model = HSICNetSparsemax(input_dim, layers, sigma_init_X, sigma_init_Y)
    ...
    L2X_scores = L2X(X, y, X, input_dim, num_samples)
